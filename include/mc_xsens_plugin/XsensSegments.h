...
{
  inline const std::string & segmentName(size_t id) { return segmentIdToName_.at(id); }
  ...
  void load(const mc_rtc::Configuration & config)
  {
    ...
    for(const auto & seg : segmentNameToId_) { segmentIdToName_[seg.second] = seg.first; }
  }
...
struct ConfigurationLoader<mc_xsens_plugin::XsensSegments>
{
  ...
  static mc_rtc::Configuration save(const mc_xsens_plugin::XsensSegments & object) { return object.save(); }
};
} // namespace mc_rtc
