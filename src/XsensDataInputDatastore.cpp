...
{
XsensDataInputDatastore::XsensDataInputDatastore(const XsensSegments & segments,
...
bool XsensDataInputDatastore::update()
{
  ...
  for(const auto & [segmentName, segmentId] : segments_.segmentNameToId())
  {
    ...
    if(ds.has("ReplayPlugin::GetSegmentVel::" + segmentName))
    {
      recvData.segment_vels_[segmentName] =
          static_cast<Eigen::Vector6d>(ds.get<Eigen::VectorXd>("ReplayPlugin::GetSegmentVel::" + segmentName));
    }

    if(ds.has("ReplayPlugin::GetSegmentAcc::" + segmentName))
    {
      recvData.segment_accs_[segmentName] =
          static_cast<Eigen::Vector6d>(ds.get<Eigen::VectorXd>("ReplayPlugin::GetSegmentAcc::" + segmentName));
    }
  ...
  if(!recvData.segment_poses_.empty()) { data_ = recvData; }
