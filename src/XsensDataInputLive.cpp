...
bool XsensDataInputLive::update()
{
  ...
  data_.comPosition_ = Eigen::Vector3d{CoMdata.pos[0], CoMdata.pos[1], CoMdata.pos[2]};
  data_.comVelocity_ = Eigen::Vector3d{CoMdata.vel[0], CoMdata.vel[1], CoMdata.vel[2]};
  data_.comAcceleration_ = Eigen::Vector3d{CoMdata.acc[0], CoMdata.acc[1], CoMdata.acc[2]};
  return true;
}
