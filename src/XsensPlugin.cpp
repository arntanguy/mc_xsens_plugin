...
void XsensPlugin::init(mc_control::MCGlobalController & gc, const mc_rtc::Configuration & config)
{
  ...
  if(liveMode_)
  {
    ...
    mc_rtc::log::error_and_throw("[XsensPlugin] LIVE mode is not supported as this plugin wasn't build with "
                                 "xsens_streaming library support. Please re-build the plugin to enable this feature");
#endif
  }
  else
  {
...
void XsensPlugin::reset(mc_control::MCGlobalController & gc)
{
  ...
  ctl.datastore().make_call("XsensPlugin::GetSegmentPose",
                            [this](const std::string & segmentName) { return data_->segment_poses_.at(segmentName); });
  ctl.datastore().make_call("XsensPlugin::GetSegmentVel",
                            [this](const std::string & segmentName) { return data_->segment_vels_.at(segmentName); });
  ctl.datastore().make_call("XsensPlugin::GetSegmentAcc",
                            [this](const std::string & segmentName) { return data_->segment_accs_.at(segmentName); });
  ctl.datastore().make_call("XsensPlugin::GetCoMpos", [this]() { return data_->comPosition_; });
  ctl.datastore().make_call("XsensPlugin::GetCoMvel", [this]() { return data_->comVelocity_; });
  ctl.datastore().make_call("XsensPlugin::GetCoMacc", [this]() { return data_->comAcceleration_; });
}
