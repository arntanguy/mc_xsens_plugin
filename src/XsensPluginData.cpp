...
void XsensData::addToLogger(mc_rtc::Logger &logger, const std::string &prefix) {
  for (const auto &segment : segment_poses_) {
    ...
    logger.addLogEntry(
        fmt::format("{}_segments_{}_pose", prefix, segmentName), this,
  ...
  for (const auto &segment : segment_vels_) {
    ...
    logger.addLogEntry(
        fmt::format("{}_segments_{}_velocity", prefix, segmentName), this,
  ...
  for (const auto &segment : segment_accs_) {
    ...
    logger.addLogEntry(
        fmt::format("{}_segments_{}_acceleration", prefix, segmentName), this,
  ...
  logger.addLogEntry(
      fmt::format("{}_com_position", prefix), this,
      [this]() -> const Eigen::Vector3d & { return comPosition_; });
  logger.addLogEntry(
      fmt::format("{}_com_velocity", prefix), this,
      [this]() -> const Eigen::Vector3d & { return comVelocity_; });
  logger.addLogEntry(
      fmt::format("{}_com_acceleration", prefix), this,
